...
PlotXYColor {
	...
	*new {
		arg corpus, mouseOverFunc, headerArray /* optional */, idArray /* optional */, slewTime = 0.5, ignorePrevious = true;
		^super.new.init(corpus,mouseOverFunc,headerArray,idArray,slewTime,ignorePrevious);
	}

	init {
		arg corpus_, mouseOverFunc_, headerArray_, idArray_, slewTime_, ignorePrevious_;
		corpus = corpus_;
	...
	getScaledXYColorFromIndices { // pass in what vector indices are currently being displayed and a vector and get back the appropriately scaled values
		...
		var color = vector[colorindex].linlin(0,1,0.8,0);// because both 0 and 1 are red...
		^[dispx,dispy,color];
	}
